...
for _ in range(t):
    ...
    yIndex = []
    for i in x:
        xIndex.append(alphabets.index(i))
    for j in y:
        yIndex.append(alphabets.index(j))
    for n in range(len(xIndex)):
        if xIndex[n] <= yIndex[n]:
            answer.append(yIndex[n] - xIndex[n])
        else:
            answer.append((yIndex[n] + 26) - xIndex[n])
