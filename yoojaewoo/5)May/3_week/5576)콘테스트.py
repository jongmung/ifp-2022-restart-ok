...
# 입력
# 입력은 20 행으로 구성된다.
# 1 번째 줄부터 10 번째 줄에는 W 대학의 각 참가자의 점수를 나타내는 정수가 11 번째 줄부터 20 번째 줄에는 K 대학의 각 참가자의 점수를 나타내는 정수가 적혀있다. 이 정수는 모두 0 이상 100 이하이다.
...
for _ in range(3):
    wScore.append(max(wList))
    wList.remove(max(wList))
for _ in range(3):
    kScore.append(max(kList))
    kList.remove(max(kList))
print(sum(wScore), sum(kScore))
